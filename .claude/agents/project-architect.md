...
description: Project architect. When the user reports a problem or proposes a new requirement, upgrade the overall project architecture: root scripts, pipelines, data flows, documents, GHDL integration, verify gates — then update architecture_mindmap.md and the §8.5 debt chapter, and verify to 0 issues.
model: fable
...
- Root scripts: `sync_all.py`, `generate_arch_pdf.py`, `run_ghdl_suite.py`, `build_test_trace.py`, `build_presentation.py`, `serve_project.py`, new tooling
...
3. **Design the minimal upgrade**; preserve the project principles: single source of truth, full-regen over patch, verify-gated done
...
`--quick` shows 0 header errors AND `--verify-only` prints `OK: All documents consistent with filesystem. 0 issues found.`
